...
- **M11.5 Notifications + shell v2:** notifications schema/router (typed events, capability-derived recipients, hardened fan-out — B1/B4/X3) + left-sidebar shell + top bar with bell/panel; workspace cockpit unchanged inside shell.
  - ⚠️ **Postmortem (fixed 2026-07-29, migration 0022):** the dedupe index shipped PARTIAL (`WHERE dedupe_key IS NOT NULL`), which Postgres cannot infer for `ON CONFLICT (recipient_id, dedupe_key)` — so **every pipeline notification upsert failed with 42P10 from M11.5 until 0022**, silently, because the writers were best-effort and discarded the error. No pipeline card ever reached the bell. Found by the M12.1 adversarial review, reproduced against the live DB. Lesson for future writers: `ignoreDuplicates` upserts need a NON-partial unique index, and "best-effort" must still LOG the error — a swallowed error is an invisible outage.
...
- **M11.7 Email infrastructure (Resend) [PRATIK: provision Resend + domain]:** route Supabase auth emails (confirm/reset/magic-link) through Resend SMTP on the brand domain; notification emails driven by the same events as the bell (immediate for approvals, daily digest otherwise); per-user email preferences page (toggle per notification kind); templates render server-side from event data only (B1 discipline); SPF/DKIM/DMARC per the M12.3 GAP list.
  - ✅ Wired env-gated (2026-07): zero-dep Resend transport + escaped brand templates in `packages/shared/src/email/`; `profiles.email_notifications` + `update_own_profile()` definer (0018/0019); `/settings` page (name, email toggle, password reset via recovery link → `/auth/reset-password`); invite emails from `invites.create`; immediate identity-review emails from the pipeline to opted-in underwriter+ recipients. All sends are visible no-ops until `RESEND_API_KEY` + `EMAIL_FROM` land in Vercel + Trigger.dev.
  - ✅ Daily digest shipped: `packages/pipeline/src/trigger/daily-digest.ts` (Trigger schedule, 13:00 UTC) — one email per active, opted-in recipient summarizing UNREAD notifications from the last 24h, excluding approval-class kinds (they mail immediately; a digest must never be the first notice of a pending approval, and no event is sent twice). Never mutates notification state — the bell stays the record. **Deploy-env fix in the same PR:** `trigger.config.ts` syncEnvVars never forwarded `RESEND_API_KEY`/`EMAIL_FROM`/`NEXT_PUBLIC_APP_URL`, so worker email would have been a silent no-op in prod even with the key set locally.
  - ✅ **LIVE 2026-07-29**: Resend key provisioned; auth SMTP configured via Management API (smtp.resend.com:465, sender notifications@credexis.co); Supabase `site_url`/`uri_allow_list` moved off localhost-only to the production origin (prod password resets would otherwise have linked to localhost); worker redeployed so Trigger carries the mail env. Verified end to end — REST send returned a message id AND a real `/auth/v1/recover` delivered through Resend SMTP. See docs/environments.md § Email delivery for the config-sync rules and the quoting/URL gotchas.
  - Remaining (not blocking): per-kind preference granularity if pilot feedback wants it.
- **M12.0 RLS integration harness (GAP list; gates the borrower portal):**
  - ✅ Shipped (2026-07): `packages/schema/src/rls/` — Supabase shim (roles/auth/storage/default grants) + real-migration replay on a throwaway Postgres + impersonated-user scenarios (tenant isolation, invite tier lattice A1, no client notification inserts B1, storage path scoping, deactivation kill-switch, definer EXECUTE revokes). Dedicated CI job `rls-harness` with a postgres:17 service container; suite skips without `RLS_HARNESS_DATABASE_URL`. Standing rule now enforceable: every new policy lands with a scenario here.
...
- **M12.1 Borrower portal:** design APPROVED — see `docs/design/platform/05-borrower-portal.md` (3 independent architectures, 36 adversarial attacks, synthesized build plan). **Spine: no-profile / invite-bound** — a borrower is an `auth.users` row with NO `profiles` row ever; authority is `borrower_invites.auth_user_id = auth.uid()`. They match exactly TWO policies in the whole database (both on `storage.objects`) and ZERO in `public`. Every existing tenant policy is left untouched and is vacuously false for them (`current_tenant_id()` → NULL). Disjointness (member XOR borrower) is enforced in three definers so the argument is provable, not aspirational. Build order: 7 PRs, nothing user-reachable until PR 5, no bytes accepted until PR 6.
  - ✅ **PR 3 — claim flow + disjointness (2026-07-29, migration 0030)**: `claim_borrower_invite()` (token digest compared, never stored; **the invited mailbox is the second factor** — an attacker holding a leaked link who signs in as themselves is refused, so the link alone is worthless; single-seat; idempotent re-claim), `borrower_portal_state()` (the ONLY read a borrower performs — curated status derived from borrower-visible facts, **never `deals.status`**, so internal pipeline state cannot leave the DB; item satisfaction counts only this invite's own uploads; per-file state is two-valued so a borrower never learns whether extraction ran), `borrower_attach_upload()` (authoritative byte size read from the object itself so a caller cannot understate to evade quotas; **fails closed** if the upload was never finalized; the only way a borrower-originated `documents` row is born). **Disjointness is now enforced in all three entry points** — `create_organization` and `accept_invite` were CREATE OR REPLACE'd with their live bodies plus one guard each, so a borrower can never acquire a profile and turn `current_tenant_id()` non-NULL. 11 harness scenarios including the leaked-link attack, single-seat, both disjointness directions, and the exact curated key set.
  - ✅ **PR 2 — access path (2026-07-29, migration 0029)**: the borrower's entire authority. `current_invite_ids()`/`has_borrower_invite()` definers (borrowers can never READ `borrower_invites`, only be resolved by it; liveness re-checked every statement so revocation bites immediately, not at token refresh); `borrower_upload_key_ok()` validating EVERY path segment against the caller's own invite (uuid::text = segment, never segment::uuid — a cast error inside a policy is both a DoS and an oracle); `borrower_object_budget_ok()` closing bucket flooding that row quotas miss; the TWO storage policies (insert + select, no update/delete — borrower objects immutable). Also tightened four reference policies that were `USING (true)` — written before a profile-less identity class existed, they would have leaked the taxonomy/registry/policy packs/global mappings to any borrower. 12 harness scenarios: segment forgery (tenant/deal/invite), prefix escape, malformed shapes, read isolation, immediate revocation, expiry, budget, and an org-user regression proving staff access is byte-identical.
    - **Platform constraint found and worked around honestly**: `storage.objects` is owned by `supabase_storage_admin`; `postgres` may manage its POLICIES but not its structure, so the design's supporting index cannot be created (42501 via every route, including a role-grant attempt Supabase reserves for superusers). The design anticipated this — correctness does not depend on the index, only prefix-count speed. Recorded in the migration rather than silently dropped.
  - ✅ **Borrower identity (2026-07-29, migrations 0027/0028)** — a borrower is a PERSON the org knows, not an email on one deal: `borrowers` (full_name + email, both required) reused across every deal they appear on, which is what makes real onboarding, repeat business and contact history possible. **Uniqueness is `(tenant_id, lower(email))` — per-tenant, deliberately NOT global**: two lenders may legitimately share a borrower, and a global constraint would leak that another tenant holds them ("email already exists" as an existence oracle). `full_name` is load-bearing, not paperwork — it is the deterministic prior the M11.6 matcher scores printed document names against. Invites now carry `borrower_id` (one live invite per borrower per deal) while keeping their own bound-email snapshot, so correcting a contact typo can never silently re-target a live claim link — a guard refuses it and demands revoke-and-re-mint. 6 harness scenarios including the cross-tenant case.
  - ✅ **PR 1 (2026-07-29)** — schema floor, deliberately unreachable: `borrower_invites` + `document_requests` + `documents.uploaded_via_invite_id` (migrations 0025/0026) with org-side RLS only. No borrower policy, helper or definer exists yet. Dedupe index swapped from deal-wide to **writer-scoped** (`documents_deal_sha256_scope_uq`), closing the 409 existence oracle that would let a borrower probe whether the lender already holds a file. Column-level UPDATE grant keeps `deal_id`/`email`/`token_hash`/`auth_user_id`/`claimed_at` definer-only — RLS cannot restrict columns. 7 harness scenarios, including one proving a signed-in user with NO profile (the borrower shape) reads zero rows from every relevant table.
    - **Deviation from the design's PR split, recorded:** the design put policies in its own PR, but standing order #6 flags any table with RLS and no policy as dead-locked, so the org-side floor ships with the DDL — the pairing every prior table in this repo uses.
- **M12.1 Borrower portal (original scope):** magic-link invites scoped to (deal, entity); upload-only RLS with full path pinning (B2/B3); curated status tracker; checklist-driven uploads; T+7 reminder; upload quotas + AV-before-extraction.
  - ✅ Prerequisites shipped (2026-07): **AV verdict enforced before extraction** — `StructuralScanner` (magic-byte ⇔ declared mime; PDFs rejected for /Encrypt, /JavaScript, /JS, /Launch, /EmbeddedFile) wired into the ingest `VirusScanner` seam, verdict + engine recorded on `documents.virus_scan`; extraction double-locked (requires `virus_scan='clean'` on the row, independent of the ingest throw path). **Per-deal upload quotas** — `tenants.settings.limits` (policy-as-data, defaults in `@credexis/shared` limits.ts: 60 docs / 1 GiB) enforced in the upload route AND by migration 0021's BEFORE INSERT trigger (backstop no caller skips — RLS-harness scenarios prove even superuser hits it). **Per-deal cost ceiling** — Σ `extraction_runs.cost_micro_usd` vs `maxCostMicroUsdPerDeal` ($10 default = the costs-page envelope) checked before extraction; withheld work surfaces as a failed run row + `document_failed` notification (writer added). Signature AV (ClamAV sidecar) behind the same port remains listed for pilot hardening.
...
- **M12.3 Vendor-security GAP list:**
  - ✅ CSP + security headers (2026-07-29): nonce-based `script-src` with `strict-dynamic` (never `unsafe-inline`), origins derived from env, HSTS/nosniff/DENY-framing/strict-origin-referrer/Permissions-Policy/COOP, applied on every middleware exit path including the rate-limit 429. `apps/web/src/lib/security-headers.ts`.
  - ✅ Audit-log tamper evidence (2026-07-29, migrations 0023/0024): per-tenant sha256 hash chain assigned by a BEFORE INSERT trigger (advisory-locked per tenant so concurrent writers cannot fork it); `verify_audit_chain(tenant)` returns the first broken link. Append-only revokes stop the app from rewriting history; the chain proves to an examiner that nobody with DB credentials did either. **Honest limit:** rows written before 0024 were backfilled so the chain is contiguous — a backfilled hash attests to the row as it exists now, so tamper evidence is meaningful only from 0024 forward. RLS-harness scenarios prove detection of both alteration and deletion.
  - Remaining: session timeouts (idle/absolute), login/invite lockout, auth event logging, PII-at-rest (EIN/SSN-last4) decision, retention/deletion + tenant offboarding export, email SPF/DKIM/DMARC + provider DPA + subprocessor list. (RLS harness as a CI gate ✅ M12.0; rate limits ✅ M10.3 for /api writes.)
- **M12.4 Email digests + document-request messaging.**

## M13 — Classifier structural guards (first full-deal walkthrough fallout)

- **M13.1 Classifier claims are structural, never invented:** the first full-deal walkthrough (docs/TEST_FINDINGS_2026-07-30.md) proved the deterministic layer held while the LLM fallback invented identities — a 14-page 1120 return read its line-12 citation "attach Form 1125-E" as the page's own identity (which would have silently dropped revenue, COGS and net income from the spread), and the seven pages of Form 4626 — a family we do not support — were relabelled 4562/1120. Structural fixes, because instructions alone demonstrably did not hold: the deterministic layer's form tokens must be header-anchored, non-cited identity — "(attach Form 4562)" is a reference context and never classifies (#177); the LLM must report the form number printed as the page's OWN header, and the CODE maps print → family through a fail-closed table (`validateLlmClaim`) — an unknown print abstains, never snaps to the nearest known form (#179). New honesty families: 4626 as known-but-unsupported with a deterministic pattern (its pages never reach the LLM) and NON_FORM for cover sheets/separators; extract-stage skips both by design — labelled, not extracted.
  - ✅ Shipped (2026-07-30, #177 + #179): 770 unit tests; signal-sweep 71 docs / 1,168 pages, 0 wrong 0 suspect. The sweep itself — 69 public IRS docs adversarially testing the splitter — landed the same day (#178) and is the standing regression harness for anything touching classification.
- **M13.3 Orphan-document sweeper:** borrower uploads landed in storage with nothing to enqueue ingest (the borrower deployment is deliberately denied Trigger.dev credentials). A staff-side cron (`sweep-orphan-documents`, every 10 min) enqueues ingest for documents with no extraction run.
  - ✅ Shipped (2026-07-30, #181): rescued its first real document in production the same day.
- **M13.4 Schedule L spreads the Balance Sheet:** business-return registries map Schedule L to `bs.*` taxonomy nodes for 1120, 1120-S and 1065 — END-OF-YEAR column only (the beginning column is the prior period, whose honest source is a prior-year upload). The 1065's divergent numbering respected; each form carries a registry relation asserting total assets = total liabilities & equity.
  - ✅ Shipped (2026-07-31, #184): line numbering verified against the official 2023 printed PDFs.
- **Scanned PDFs are readable — the vision path lives:** the first real customer return (a 19-page scan, zero characters of text) came back as one undifferentiated span with no facts and no error. Three defects: nothing ever rendered page images for the tested vision branch; pdf.js TRANSFERS its input buffer, so reading text consumed the file; rendering failures were swallowed by a bare catch. Pages with no usable text layer now render to PNG for the vision classifier (text-bearing pages never do, so native PDFs cost what they did before). Follow-up hardening: page images bind by identity — the page is named BEFORE its image, never inferred from position (Iron Law #4); batches close on image count; a thrown batch degrades to unresolved instead of rejecting the bundle; a document whose pages neither carry text nor render is status=failed, never a silent success.
  - ✅ Shipped (2026-07-31 #186, 2026-08-01 #187): verified against the real customer file — 1 span → 7 spans across 1120-S/K-1/1125-E/4562, and it corrected the form family (the return is an 1120-S, not the 1120 it was labelled); #187 verified in production on a text-less fixture.

## M14 — Golden-deal fixes (the first real bank package)

- **M14 Golden Deal 1 through the pipeline:** each fix came from running the real Travelodge Merrill package (29 documents, ~480 pages) and is locked as a regression:
  - ✅ Debt schedules outrank the balance sheets they reference (2026-08-05, #188): the standard debt-schedule template instructs that balances "should match the current balance sheet", and the statement keywords tested BALANCE_SHEET first. Most-specific-first — a balance sheet never says "debt schedule"; the reverse is routine.
  - ✅ Garbled ToUnicode text layers route to vision (2026-08-05, #189): a QuickBooks-printed balance sheet extracted as control characters and remapped code points while rendering perfectly. Detector: >5% C0/private-use/replacement characters counts as no usable text layer — the page renders for the vision classifier with its garbage text blanked, so the model reads pixels, not soup.
  - ✅ Duplicate learned mappings no longer fail statement extraction (2026-08-10, #191, migration 0035): Postgres UNIQUE treats NULLs as distinct, so GLOBAL (tenant NULL) write-backs stacked 16 copies of "total assets" and `.maybeSingle()` threw — zero facts from the Travelodge P&L. Index recreated NULLS NOT DISTINCT, existing dupes deduped (human beats llm, then usage), and reads pick the best candidate instead of demanding uniqueness.
  - ✅ Business returns extract back to TY2020 (2026-08-10, #192): real deals carry three years of returns; the registry started at 2023. Verified line-by-line against official printed revisions — the 1065's 2023 §179D renumbering was wrong in the base data (the [PRATIK] caveat, now earned and corrected); TY2020–2022 land via derived overrides; G4 gate specs filter by the period's FY label so a pre-2023 subset relation cannot false-positive on a 2023 return.

## M14.5 — Extraction follows entity assignment (multi-entity deals unblocked)

- **M14.5 Extract on assignment:** multi-entity deals skip extraction at ingest by design (no entity to bind facts to), but the assignment mutation updated the row and STOPPED — a deal with a target and two guarantors, the Golden Deal's exact shape, could never produce facts. New `extract-document` Trigger task enqueued by `assignment.assign` whenever an entity is set: per-span payload keeps concurrent assignments on disjoint work, an (span, entity) idempotency key and zero-facts guard make repeat-assigns no-ops, the ingest path's AV lock is enforced identically, and the deal auto-advances to review exactly like the ingest path. The assignment UI says "extraction queued" so facts arriving a minute later don't look like magic.
  - ✅ Shipped (2026-08-11, #194 — the squash carried the work after #193 auto-closed empty; see the stacked-branch trap in docs/HANDOFF.md).

## M15 — Pro-forma engine (the banker's workbook as a module)
